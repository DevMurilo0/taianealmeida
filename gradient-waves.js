/*
 * GradientWaves — vanilla JS adaptation (no React) of the React Bits component,
 * mounted as a decorative background inside the "A Autora" section only.
 *
 * The original component (from React Bits) is a React wrapper around a small
 * ogl (WebGL2) raymarched wave shader. Since this project has no React/JSX
 * build step, the same rendering logic below is ported to plain JS and
...
function mountGradientWaves(container, options = {}) {
  ...
  // Respect users who prefer reduced motion — keep the section fully static.
  ...
  // The container itself is pointer-events:none (per spec), so mouse
  // parallax is driven by the parent section, which still receives events.
  const parallaxTarget = container.closest('.autora') || container;
  ...
  if (opts.mouseInteraction) {
    ...
    parallaxTarget.addEventListener('pointerleave', onPointerLeave);
  }
